...
public class ObjectsDetector {
	...
	public ObjectsDetector() {
		...
		try {
			...
			model = SavedModelBundle.load(args[0], "serve");
	...
	public DetectionResult detectObjects(BufferedImage img) {
		...
		try (Tensor<Float> scoresT = outputs.get(0).expect(Float.class);
			...
			//System.out.printf("* %s\n", filename);
			DetectionResult result = new DetectionResult(scores, classes, boxes, 0);
	...
	private static void printSignature(SavedModelBundle model) throws Exception {
		...
		for (Map.Entry<String, TensorInfo> entry : sig.getOutputsMap().entrySet()) {
			TensorInfo t = entry.getValue();
			System.out.printf("%d of %d: %-20s (Node name in graph: %-20s, type: %s)\n", i++, numOutputs,
					entry.getKey(), t.getName(), t.getDtype());
		}
		System.out.println("-----------------------------------------------");
	}
